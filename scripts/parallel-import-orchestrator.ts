...
class ProgressTracker {
  ...
  displayProgress(): void {
    ...
    console.log(`Chapters: ${this.stats.completedChapters}/${this.stats.totalChapters} (${this.stats.failedChapters} failed)`);
...
class ParallelImporter {
  ...
  private async fetchChapterWithRetry(
    ...
    try {
      ...
      if (attempt < this.config.maxRetries) {
        console.error(`  ⚠️  Retry ${attempt}/${this.config.maxRetries} for ${bookId} ${chapterNumber}`);
        await new Promise(resolve => setTimeout(resolve, this.config.retryDelay));
        return this.fetchChapterWithRetry(apiPath, bookId, chapterNumber, translation, attempt + 1);
      } else {
        console.error(`  ❌ Failed ${bookId} ${chapterNumber} after ${this.config.maxRetries} attempts:`, error);
  ...
  async importVolume(volume: VolumeConfig): Promise<Book[]> {
    ...
    for (const bookInfo of volume.books) {
      const promise = bookPool.add(() =>
        this.importBook(volume, bookInfo, chapterPool)
      );
...
async function generateVolumeManifest(
  ...
  if (existingManifest) {
    ...
    console.log(`📝 Merging with existing manifest (${existingManifest.books.length} existing + ${uniqueNewBooks.length} new books)`);
  }
...
async function main() {
  ...
  try {
    ...
    for (const [translationId, books] of results.entries()) {
      ...
      else if (translationId === 'kjv') volumeConfig = source === 'ot' ? OLD_TESTAMENT : NEW_TESTAMENT;
